...
import type { DashboardPayload, InstalledSkillsScopeState } from '../features/skills/state';
...
export function DashboardPage() {
  ...
  const totalInstalledSkills = dashboardPayload
  ...
  const loadInitialDashboardState = async () => {
    ...
    try {
      ...
      setUnexpectedRefreshError(`Dashboard load failed: ${message}`);
    } finally {
  ...
  const handleRefresh = async () => {
    ...
    try {
      const refreshed = await refreshDashboardState(dashboardPayload.installedState);
  ...
  if (isInitialLoading && !dashboardPayload) {
      ...
      <main className="min-h-svh bg-background">
        <div className="mx-auto flex w-full max-w-2xl flex-col gap-4 p-4 sm:p-6">
          <Card className="border shadow-none">
            <CardHeader>
              <CardTitle className="text-base">Unable to load dashboard</CardTitle>
              <CardDescription>
                {unexpectedRefreshError ?? 'No data is available yet.'}
              </CardDescription>
            </CardHeader>
            <CardContent>
              <Button onClick={() => void loadInitialDashboardState()} size="sm">
                Retry
              </Button>
            </CardContent>
          </Card>
