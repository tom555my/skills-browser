...
import { useDotMatrixPhases, usePrefersReducedMotion } from '@/web/lib/dotmatrix-hooks';
...
function buildSnakeOrderToIndexMap(): number[] {
  const pathOrder = Array.from({ length: CELLS }, () => 0);
...
function buildSpiralInwardOrderToIndexMap(): number[] {
  const order = Array.from({ length: CELLS }, () => 0);
  let top = 0;
  let bottom = N - 1;
  let left = 0;
  let right = N - 1;
...
function buildOuterRingClockwiseOrderToIndexMap(): number[] {
  const order = Array.from({ length: CELLS }, () => -1);
